...
if ( ! function_exists('get_phrase'))
{
	function get_phrase($phrase = '') {
		...
		$langquery	=	$CI->db->get_where('language' , array('phrase' => $phrase));
		$rowlang   	=	$langquery->row();	
		
		// return the current sessioned language field of according phrase, else return uppercase spaced word
		if (isset($rowlang->$current_language) && $rowlang->$current_language !="")
			return $rowlang->$current_language;
		else 
