...
<?php 
    $mail_details = $this->db->get_where('action',array('action_id'=>$action_id))->row_array();
 ?>
<!-- Page Sidebar Ends-->
<div class="page-body">
                                ...
                                <div class="email-body">
                                    <div class="email-compose">
                                        ...
                                        <div class="email-wrapper">
                                            <form class="theme-form" action="<?php echo base_url(); ?>admin/actions/resend_mail" method="POST" enctype="multipart/form-data">
                                                <input type="hidden" name="action_type" value="<?php echo $mail_details['action_type']; ?>">

                                                <div class="col-sm-12 col-md-12 ">
                                                    <div class="mb-3">
                                                        <label class="form-label" for="exampleFormControlSelect9">Para</label><span style="color:red">*</span>
                                                        <select class="form-select js-example-basic-single" required name="directory_id" readonly>
                                                            <option value="">Seleccionar</option>
                                                            <?php  $managers = $this->db->get_where('directory', array('status'=>1, 'email !='=>''))->result_array();
                                                                foreach ($managers as $mg): ?>
                                                            <option value="<?php echo $mg['directory_id'] ?>" <?php echo $mail_details['send_to'] == $mg['directory_id'] ? "selected":""; ?>><?php echo $mg['name'].' '.$mg['first_last_name'].' '.$mg['second_last_name']?></option>
                                                            <?php endforeach;  ?>
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    ...
                                                    <input class="form-control" id="exampleInputPassword1" type="text" name="subject" value="<?php echo $mail_details['razon']?>">
                                                </div>
                                                <div class="form-group">
                                                    <label>Mensaje</label>
                                                    <textarea id="text-box" name="message" cols="10" rows="2" name="message"><?php echo $mail_details['message']?></textarea>
                                                </div>
                                                <div class="default-according style-1  faq-accordion job-accordion  col-xl-12">
                                                    <div class="card">
                                                        ...
                                                        <div class="collapse <?php echo $mail_details['attachments'] != ""? "show":"" ?> " id="collapseicon2" aria-labelledby="collapseicon2" data-parent="#accordion" style="">
                                                ...
                                                <div class="action-wrapper">
                                                    <button class="btn btn-primary once-only" type="submit"><i class="fa fa-file me-2"></i> Reenviar</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
...
var firstUpload = new FileUploadWithPreview('myFirstImage', {
    ...
    presetFiles: [<?php $arreglo = explode(",",$mail_details['attachments']);
    for($i=0;$i<count($arreglo);$i++){
        if($arreglo[$i] != "")
        echo '"'.base_url().'public/uploads/emails/'.$mail_details['action_id'].'/'.$arreglo[$i].'",'; //imprimimos cada nombre 
    }
     ?>],
    multiple: true,
})
